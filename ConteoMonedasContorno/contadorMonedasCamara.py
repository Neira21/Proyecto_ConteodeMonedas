...
def ordenarPuntos(puntos):
    #La funcion concatenate es unir o enlazar matrices
    #El punto .toList() transforma en una lista
    n_puntos=np.concatenate([puntos[0],puntos[1],puntos[2],puntos[3]]).tolist()
    
    #Ordenar los n puntos, key: asume el tipo de orden que se da a la matriz
    #n_puntos[1] se le resta la unidad
...
# Para cuando apuntemos el celular al objeto a analizar, si rotamos o mantenemos el equiPO
# el reconocimiento de las imágenes va a ser el mismo
...
while True:
    ...
    if cv2.waitKey(1)==ord("q"):
        break
    imagen_A6 = alineamiento(Camara, ancho=480, alto = 667)
